    ...
    render() {
        ...
        return (
            <div style={{paddingTop:"40px",marginLeft:"400px"}}>
                ...
                {this.state.orders.map((order)=> <Table.Row key={order.id} order={order}>
                            <Table.Cell><h2>{order.id}</h2></Table.Cell>
    <Table.Cell><h3>{order.clothings.map((clothing)=><h4>{clothing.name}</h4>)}</h3></Table.Cell>
                            <Table.Cell><h3>{order.total}$</h3></Table.Cell>
                </Table.Row>)}
                </Table.Body>
          </Table>
            </div>
            ...
            return(
                 <div><h1>You haven't ordered yet</h1></div>
            )
        }
    }
}
